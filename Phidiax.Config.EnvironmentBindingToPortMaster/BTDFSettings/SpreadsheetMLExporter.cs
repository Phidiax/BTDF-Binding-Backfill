...
namespace Phidiax.Config.EnvironmentBindingToPortMaster
{
    ...
    {        
        ...
        {
            var stream = new System.IO.MemoryStream(System.Text.Encoding.ASCII.GetBytes(Properties.Resources.EnvironmentSettingsTemplate));
            var doc = new XmlDocument();
            var nm = new XmlNamespaceManager(doc.NameTable);
            nm.AddNamespace("ss", "urn:schemas-microsoft-com:office:spreadsheet");
            doc.Load(stream);
            var table = doc.SelectSingleNode("//ss:Table", nm) as XmlElement;
